...
const gOut_two_controller = require("../../Model/locaTwo_schema/Gout");

const g_out_controller = {
    index : async (req, res) => {
        const g_outPost = await gOut_two_controller.find().sort({ createdAt : -1});
    ...
    store : async (req, res) => {
        try {
            ...
            const UserPost = await gOut_two_controller.create({
                Id,
                Ag,
                Name,
                Phone,
    ...
    show : async (req, res) => {
        try {
            ...
            const gPost = await gOut_two_controller.findById(id);
            if (!gPost) {
    ...
    destory : async (req, res) => {
        try {
            ...
            const gPost_deleted = await gOut_two_controller.findByIdAndDelete(id);
            if (!gPost_deleted) {
    ...
    update : async (req, res) => {
        try {
            ...
            const G_update = await gOut_two_controller.findByIdAndUpdate(id, {...req.body});
            if (!G_update) {
