...
export default function Accounting() {
    ...
    useEffect(() => {
        const fetchAccount = async () => {
            const response = await fetch('http://localhost:8000/api/cashier_three');
  ...
  return (
    <>
    <div>
        <h3 className='text-base font-semibold p-2 bg-gray-900 text-white mb-2 cursor-pointer'>ငွေကိုင်</h3>
        {
            Array.isArray(account) && account.length > 0 ? (account.map((account_data, index) => (
                <Link to={`/Cashier3/Edit/${account_data._id}`} key={account_data._id}>
                    <p className={`
                        rounded-md m-1
