...
export default function Car_order3_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`http://localhost:8000/api/carOrder_three/${id}`);
                    ...
                    setError('Error fetching sale_out in data', error);
                };
            };
            fetch_api();
        };
    ...
    const UpdatePost = async (e) => {
        try {
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/carOrder_three/${id}`, addPost);
            } else {
                res = await axios.post(`http://localhost:8000/api/carOrder_three`, addPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                };
            };
        } catch (error) {
            setError('Faild UpdatePost', error);
        };
    };
                    ...
                    {isOpenVisible && (
                        <div className='absolute mt-2 w-40 p-4 bg-white shadow-lg rounded-lg select-none'>
                            <ul>
                                ...
                                <li onClick={openAddNew} className="py-2 px-4 text-gray-700 cursor-pointer hover:bg-sky-100">Add New</li>
                                <li onClick={openVisible} className="py-2 px-4 text-gray-700 cursor-pointer hover:bg-sky-100">Cancel</li>
                            </ul>
                        </div>
                    )}
                </div>
            ...
            {Array.isArray(data) && data.length > 0 ? (
                data.map((item, index) => (
                    <div key={index} className='pt-6'>
                        ...
                        <div className='flex justify-between space-y-6 items-center'>
                            <p className='text-gray-400 font-semibold'>ဌာနတည်နေရာ</p>
                            <p>{item.selection}</p>
                        </div>
