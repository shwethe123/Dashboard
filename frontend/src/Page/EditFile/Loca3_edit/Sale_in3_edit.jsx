...
export default function Sale_in3_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`http://localhost:8000/api/salesIn_three/${id}`);
    ...
    const UpdatePost = async (e) => {
        try {
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/salesIn_three/${id}`, addPost);
            } else {
                res = await axios.post(`http://localhost:8000/api/salesIn_three`, addPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                }
            };
