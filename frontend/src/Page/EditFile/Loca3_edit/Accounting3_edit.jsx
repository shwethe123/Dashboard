...
export default function Accounting3_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`http://localhost:8000/api/cashier_three/${id}`);
                    ...
                    setError('Error fetching accounting in data', error);
                };
            };
            fetch_api();
        };
    ...
    const UpdatePost = async (e) => {
        try {
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/cashier_three/${id}`, addPost);
            } else {
                res = await axios.post(`http://localhost:8000/api/cashier_three`, addPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                }
            };
        } catch (error) {
            setError('Faild Updated', error);
        };
    };
