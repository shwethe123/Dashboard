...
export default function Out_juty() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`http://localhost:8000/api/outjuty/${id}`);
    ...
    const UpdatePost = async (e) => {
        try {
            e.preventDefault();
            if (!Id || !Ag || !Phone || !Name || !condition || !selection || !reason) {
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/outjuty/${id}`, addPost);
            } else {
                res = await axios.post(`http://localhost:8000/api/outjuty`, addPost);
                navigate('/LocaOne');
            }
        } catch (error) {
            setError('Falid Updated and AddPost');
        };
    };
