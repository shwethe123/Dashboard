...
export default function G_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const responses = await axios.get(`http://localhost:8000/api/dashboard/${id}`);
                    ...
                    console.log(error);
                    setError('Not response api', error);
                }
            };
            fetch_api();
        };
    ...
    const UpdatePost = async (e) => {
        try {
            e.preventDefault();
            if (!Id || !Ag || !Name || !Phone || !selection || !condition || !reason) {
                setError('အချက်လက်ထည့်ပါ')
            }else{
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/dashboard/${id}`, AddPost);
            } else {    
                res = await axios.post('http://localhost:8000/api/dashboard', AddPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                }
            };
    ...
    <>
        <div className='max-w-5xl mx-auto p-16 mt-10 bg-white rounded-lg shadow-lg'>
            <div className='flex justify-between items-center'>
                ...
                <div>
                    ...
                    {isOpenVisible && (
                        <div className='absolute mt-2 w-40 p-4 bg-white shadow-lg rounded-lg border-2 border-gray-100 select-none'>
                            <p onClick={openModal} className='py-2 px-4 text-gray-700 cursor-pointer hover:bg-sky-100 hover:text-sky-500'>Edit</p>
                            <p onClick={newCreate} className='py-2 px-4 text-gray-700 cursor-pointer hover:bg-sky-100 hover:text-sky-500'>Add New</p>
                            <p onClick={openVisible} className='py-2 px-4 text-gray-700 cursor-pointer hover:bg-sky-100 hover:text-sky-500'>Cancel</p>
                        </div>
                    )}
                </div>
            </div>
        ...
        {isUserModal && (
            ...
            <form action="" className='bg-white w-5/12'  onSubmit={UpdatePost}>
                ...
                {error && (
                    <>
                    <div className='bg-red-100 border border-red-400 text-red-500 px-4 py-3 rounded relative' role='alert'>
