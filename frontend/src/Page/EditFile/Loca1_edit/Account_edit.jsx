...
export default function Account_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`http://localhost:8000/api/accounting/${id}`);
    ...
    const UpdatePost = async (e) => {
        ...
        try {
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/accounting/${id}`, addPost);
            } else {
                res = await axios.post(`http://localhost:8000/api/accounting`, addPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                }
            }
    ...
    <>
        <div className='max-w-5xl mx-auto p-16 mt-10 bg-white rounded-lg shadow-lg'>
            ...
            {Array.isArray(data) && data.length > 0 ? (
                ...
                <div className='bg-red-100 border border-red-400 text-red-500 px-4 py-3'>!No Data</div>
        )}
        ...
        {isUserModal && (
            ...
            <form action="" className='bg-white w-5/12'  onSubmit={UpdatePost}>
                ...
                <h3 className='text-gray-600 text-2xl font-bold text-center'> {id? 'Edit' : 'Create'} Information</h3>
                <div className='m-16'>
                {error && (
