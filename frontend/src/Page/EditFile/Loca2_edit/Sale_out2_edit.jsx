...
export default function Sale_out2_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`http://localhost:8000/api/saleOut_two/${id}`);
    ...
    const UpdatePost = async (e) => {
        try {
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/saleOut_two/${id}`, addPost);
            } else {
                res = await axios.post(`http://localhost:8000/api/saleOut_two`, addPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                };
            };
