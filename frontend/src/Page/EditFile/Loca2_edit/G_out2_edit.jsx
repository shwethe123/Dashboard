...
export default function G_out2_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`http://localhost:8000/api/gout_two/${id}`);
                    if (response.status === 200) {
                        const fetchedData = response.data;
                        setId(fetchedData.Id || '');
                        setAg(fetchedData.Ag || '');
                        setName(fetchedData.Name || '');
                        setPhone(fetchedData.Phone || '');
                        setCondition(fetchedData.condition || '');
                        setSelection(fetchedData.selection || '');
                        setReason(data.reason || '');
                        setData([fetchedData]);
                    }
                } catch (error) {
                    setError('Error fetching g_out in data');
                }
            };
            fetch_api();
        }
    ...
    const UpdatePost = async (e) => {
        try {
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/gout_two/${id}`, addPost);
            } else {
                res = await axios.post(`http://localhost:8000/api/gout_two`, addPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                }
            }
