...
export default function Sale_in2_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`http://localhost:8000/api/salesIn_two/${id}`);
                    ...
                    setError('Error fetching sale_in in data', error);
                };
            };
            fetch_api();
        };
    ...
    const UpdatePost = async (e) => {
        try {
            ...
            let res;
            if (id) {
                res = await axios.patch(`http://localhost:8000/api/salesIn_two/${id}`, addPost);
            } else {
                res = await axios.post(`http://localhost:8000/api/salesIn_two`, addPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                }
            }
