...
export default function LonmoWifi() {
  const [getTime, setGetTime] = useState([]);
  const [selectedId, setSelectedId] = useState(3333);
    ...
    <div>
      {loading ? (
        ...
        <div className="">
          {getTime.length > 0 ? (
            getTime.map((item) => (
              <div key={item._id} className="flex justify-between bg-white text-black pl-3 pt-3 pb-3 rounded-xl shadow-sm hover:shadow-md">
                <div className="mb-4 sm:mb-0">
                  ...
                  <p className="text-gray-400 w-28 font-semibold text-sm">
