...
export default function TbLinkWifi() {
  const [getTime, setGetTime] = useState([]);
  const [selectedId, setSelectedId] = useState(3234);
    ...
    <div>
      {loading ? (
        ...
        <div className="">
          {getTime.length > 0 ? (
            getTime.map((item) => (
              <div key={item._id} className="flex justify-between p-2 border-2 border-purple-100 bg-white text-black pl-3 pt-3 pb-3 rounded-xl shadow-sm hover:shadow-md">
                <div>
