...
export default function Condition7() {
  const [gOutData, setGOutData] = useState([]);
  const [selectedCondition, setSelectedCondition] = useState("condition7"); // Change this as needed
      ...
      try {
        ...
        const [
          ...
          Sale_out3,
            ...
            fetch('http://localhost:8000/api/cargo_three'),
            fetch('http://localhost:8000/api/carOrder_three'),
            fetch('http://localhost:8000/api/gout_three'),
            fetch('http://localhost:8000/api/orderIn_three'),
  ...
  return (
    <div>
        <div className="flex flex-col bg-yellow-100 mt-2 p-1 rounded-md">
        <p className='mb-2 box-decoration-clone text-xs text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-pink-500 px-2 font-bold'>အလုပ်ထွက်မည့်သူစာရင်း</p>
            {gOutData.length > 0 ? (
